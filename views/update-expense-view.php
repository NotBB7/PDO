...
<div class="row justify-content-center mx-0 mb-5">
    <div class="container col-lg-8 col-10 px-lg-5 px-3 pb-5 rounded shadow bg-light">
                ...
                <div class="row justify-content-center mx-0">
                    ...
                    <div class="col-lg-6 px-3">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" class="form-control" name="date" id="date" value="<?= $_POST['date'] ?? $expense['exp_date'] ?>" required>
                        </div>
                        ...
                        <select class="form-select form-select-sm mb-4" name="type" id="type">
                            ...
                            <?php foreach (Type::getAllTypes() as $type) { ?>
                                <option value="<?= $type['typ_id'] ?>" data-tva="<?= $type['typ_tva'] ?>" <?= isset($_POST['type']) && $_POST['type'] == $type['typ_id'] ? 'selected' : ($type['typ_id']  == $expense['typ_id'] ? 'selected' : '') ?>><?= $type['typ_name'] ?></option>
                            <?php } ?>
                        </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" class="form-control" id="amount" name="amount" value="<?= $_POST['amount'] ?? $expense['exp_amount_ttc'] ?>" required>
                        </div>

                        <div class="mb-4">
                            <label for="amountHT" class="form-label">Montant HT * <i>(en €uros)</i></label>
                            <input type="text" class="form-control" id="amountHT" name="amountHT" value="<?= $_POST['amountHT'] ?? $expense['exp_amount_ht'] ?>" readonly>
                        </div>

                        <div class="mb-4">
                            <label for="tva" class="form-label">TVA * <i>(en €uros)</i></label>
                            <input type="text" class="form-control" id="tva" name="tva" value="<?= $_POST['tva'] ?? $expense['exp_amount_ttc'] - $expense['exp_amount_ht'] ?>" readonly>
                        </div>

                        <div class="mb-4">
                            <label for="description" class="form-label">Motif *</label>
                            <span class="form-error"><?= $errors['description'] ?? '' ?></span>
                            <textarea class="form-control" id="description" name="description" placeholder="ex. Déplacement chez le client ... " rows="3"><?= $_POST['description'] ?? $expense['exp_description'] ?></textarea>
                        </div>
                        ...
                        <div class="text-center">
                            <button type="submit" class="btn btn-primary mb-lg-0 mb-3">Modifier</button>
        ...
        <?php } else { ?>
            ...
            <p class="text-center h3">La note a bien été modifiée en compte.</p>
